    ...
    <Card className="bg-[#232442]/60 border-white/[0.06]">
      ...
      <CardContent className="space-y-4">
        {(["command", "governance", "specialist"] as const).map((layer) => (
          <div key={layer}>
            <p className="text-[10px] font-mono text-zinc-500 uppercase mb-2 tracking-widest">
              {layerLabel[layer]}
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5">
              {(grouped[layer] ?? []).map((agent, i) => (
                <motion.div
                  ...
                  className="flex items-center gap-2 rounded-md bg-white/[0.04] px-2.5 py-2 border border-white/[0.06] hover:border-white/[0.08] transition-colors"
                >
                  <div
